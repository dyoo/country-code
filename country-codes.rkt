...
(provide (contract-out [country-codes (listof string?)]
                       [country-code? (string? . -> . boolean?)]
                       [country-code->name (string? . -> . (or/c string? false/c))]))

;; File comes from ISO 3166:
         ...
         (lambda (matches)
           (let ([country (substring iso3166-text
                 ...
                 [code (substring iso3166-text
                                  (car (third matches))
                                  (cdr (third matches)))])
             (hash-set! ht code country)
             (loop (cdr (first matches)))))]))

(define country-codes
  (foldr cons empty (hash-map ht (lambda (k v) k))))
